...
├── runtime/             # Process-level concerns above engine/channel/kafka/queue: `tasks.rs` (the background-task supervisor behind `/readyz`), `reload.rs` (engine + channel registry + Kafka consumer, one generation), `handler_deps`
...
CLI args → config (TOML + `ORION_SECTION__KEY` env overrides) → tracing → metrics → detect DB backend from URL → DB pool + migrations → repositories (workflows, channels, connectors, packages, traces, trace_dlq, audit_logs) → ConnectorRegistry → HTTP client → engine lock (pre-created for channel_call) → cache pool → external pool caches (SQL, MongoDB) → custom functions → Kafka producer (if enabled) → load active channels + workflows → filter by include/exclude patterns → build engine → populate engine lock → reload ChannelRegistry → trace queue workers → trace cleanup → DLQ retry → Kafka consumer (if enabled, config + DB topics merged — **after** the queues, because it writes a trace per message) → rate limiter → Axum HTTP server → graceful shutdown on SIGTERM/SIGINT.
...
- **Engine hot-reload:** Engine is held as `Arc<EngineHandle>` wrapping an `ArcSwap<dataflow_rs::Engine>` (`engine/runner.rs`); the reload itself is `runtime/reload.rs`, because it reads the database, republishes the channel registry and restarts the Kafka consumer — the engine is one of three things it swaps, not the owner. A reload builds the new engine off to the side and publishes it with one atomic `store` — lock-free; readers finish on the engine they loaded — and the whole sequence is serialised by `AppStateInner::reload_lock`. Reload triggers on status changes (activate/archive), delete, and manually via `POST /api/v1/admin/engine/reload`. Draft creates/updates do not trigger reload. Also rebuilds `ChannelRegistry` and restarts Kafka consumer if topic set changed. In cluster mode the mutation then bumps the config epoch with an `EpochScope` so peers resync only what moved.
