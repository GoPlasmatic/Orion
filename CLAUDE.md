...
├── package/             # Package lifecycle shared by the `package` CLI and anything else that applies one: `verify.rs` (`PackageMembers`, `quarantined_members` — which members a generation's load issues say are not serving; structured identities only, never reason prose). A top layer beside `bootstrap`
