...
- **Admin** (`/api/v1/admin/`):
  ...
  - **Cron:** `GET /cron/occurrences[/{id}]` (the durable run ledger), `POST /cron/occurrences/{id}/retry` (another attempt at the same occurrence — same id, same `scheduled_for`), `GET /cron/status` (per-channel cursor, last run, backlog), `POST /channels/{id}/trigger` (a manual occurrence through the same claim and singleton path)
