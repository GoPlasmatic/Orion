...
├── runtime/             # Process-level concerns above engine/channel/kafka/queue: `generation.rs` (`RuntimeGeneration` = engine + channel estate, published as one value through `RuntimeHandle`), `tasks.rs` (the background-task supervisor behind `/readyz`), `reload.rs` (rebuilds that generation + the Kafka consumer), `handler_deps`
├── server/              # HTTP server, middleware, state
│   └── routes/          # admin/ (workflows, channels, connectors, plugins, cron, packages, functions, engine, audit, backups, trace_dlq), data/
...
CLI args → config (TOML + `ORION_SECTION__KEY` env overrides, `env://` references resolved after the parse) → tracing → metrics → detect DB backend from URL → DB pool + migrations → repositories (workflows, channels, connectors, plugins, cron, packages, traces, trace_dlq, audit_logs) → ConnectorRegistry → HTTP client → plugin sandbox (`WasmRuntime`, only when `plugins.enabled`; `None` makes every stored plugin a load issue on this node) → runtime handle (pre-created around an empty generation, for channel_call) → cache pool → external pool caches (SQL, MongoDB) → custom functions → Kafka producer (if enabled) → load active channels + workflows + plugins → filter by include/exclude patterns → build the FunctionRegistry (built-ins plus what loaded) + engine + channel estate → publish generation 1 → trace queue workers → trace cleanup → DLQ retry → cron reconciler + workers (if enabled) → Kafka consumer (if enabled, config + DB topics merged) — **both after the queues, because each produces work that lands in the trace pipeline** → rate limiter → Axum HTTP server → graceful shutdown on SIGTERM/SIGINT.
...
- **AppState** (`server/state.rs`): Central shared state struct. Coherent clusters are grouped into sub-structs (R26): `repos` (`storage::repositories::Repositories` — workflows, channels, connectors, plugins, cron, packages, traces, trace_dlq, audit_logs), `kafka` (producer, consumer_handle, ingest_status), and `caches` (cache_pool, sql_pool_cache, mongo_pool_cache). Runtime-singular fields stay flat: the runtime handle, the channel loader, connector registry, trace/audit queues, config, metrics handle, HTTP client, DataLogic instance, rate limit state, readiness flag, cluster runtime, admin-auth failure tracker, trusted proxies. Passed to all route handlers via Axum's `State` extractor.
...
SQLite (default), PostgreSQL, or MySQL — selected at runtime from `storage.url` scheme. All three migration sets are embedded via `sqlx::migrate!()` and the correct set is chosen at startup based on the detected backend (`DbBackend` enum in `storage/mod.rs`). `DbPool` is an enum wrapping the concrete pool types (`SqlitePool`/`PgPool`/`MySqlPool`) with dispatch helpers for query execution. Tables: `workflows` (composite PK `(workflow_id, version)`), `channels` (composite PK `(channel_id, version)`), `connectors`, `plugins` (composite PK `(plugin_id, version)`) + `plugin_artifacts` (the component bytes, by digest), `packages` (promotion receipts), `traces`, `trace_dlq`, `audit_logs`, the cron ledger (`cron_schedule_state`, `cron_occurrences`, `cron_singletons`) and the cluster tables (`config_epoch`, `job_leases`, `connector_oauth_state`); workflows, channels and connectors carry a `tags_json` column. Views: `current_workflows`, `current_channels` (latest version per ID). Triggers enforce single-draft-per-ID and active-immutability constraints. Migrations per backend in `migrations/{sqlite,postgres,mysql}/`.
...
orion-server lint ./definitions           # Validate a whole definition set and the references between its files (a plugin.toml in the tree, or --plugin-dir, makes plugin functions checkable; otherwise they are unverifiable notes)
orion-server dry-run -w wf.json -i in.json --stubs s.json --metadata m.json  # Execute a workflow offline with canned connector replies (--plugin-dir <dir>: plugin functions run for real, never stubbed)
orion-server test examples/workflow-tests # Run offline *.case.json workflow regression tests (--definitions <dir> to resolve $from/use; --plugin-dir <dir> for the components)
orion-server compile ./definitions --name p --version 1.0.0 -o dist/package.json  # Compile a definition set ($from/use resolved, plugin.toml + component inlined) into a package artifact (--format dir|bulk for POST-per-file / bulk-import shapes)
