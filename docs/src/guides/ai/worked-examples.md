...
Tested with Orion 1.9.1. You need Git, `curl`, Python 3, a POSIX shell (on Windows, WSL), and an Orion server on `http://localhost:8080`. The examples use the shipped packages, so start from a clone:
