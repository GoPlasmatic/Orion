...
It takes **no connector**. A response-cache entry can live in the default store or in any cache connector a channel names, and the author should not have to know which. The function bumps each namespace in the default store (the shared Redis in cluster mode, so every node sees it), in every in-memory response-cache store on this node, and in every Redis cache connector whose `write` gate is on. A store it cannot reach is logged and skipped. The task then fails, after every reachable store has been bumped, and the channel's `ttl_secs` bounds what the unreached store serves.
