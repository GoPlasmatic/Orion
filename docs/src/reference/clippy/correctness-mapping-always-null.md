...
Write `false` to clear a slot that later steps test, or remove the mapping.

## Caveats

Silent when the `logic` is not a compile-time constant. `{"var": "temp_data.maybe"}` may be null only at run time. `{"if": [c, x, null]}` uses `null` on one branch to mean "keep the current value", which is a legitimate reading. See [`map`](../functions/map.md) for the null rule itself.
