...
- **`orion-cli send` needs `--raw` for a payload-mode channel.** Every CLI and MCP data path wraps its argument in `{"data": …}` by default, which a payload-mode channel then delivers as `data = {"data": …}`. `orion-cli send my-channel --raw -d '…'` sends the payload verbatim; the MCP `send` tools take a `raw` parameter for the same reason. Because such a channel accepts no caller metadata, `--raw` and `--metadata` are refused together rather than one being dropped.
