...
The consequence is the one rule to design around: **if a response varies by anything a header carries, that thing must appear in the payload, or the channel must not cache.** A channel whose workflow reads `metadata.headers` — through `validation_logic` or a task — and lets a header change the response body is a channel whose cache will serve one caller's body to another. Fold the distinguishing value into the payload and name it in `cache_key_fields`, or leave `cache` disabled.
...
## How hot reload swaps the engine
