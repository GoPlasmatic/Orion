...
It reads the graph: parameter and node counts, IR version and opset. The parameter count is every value the graph carries, in its initializers or in its nodes' attributes, through every subgraph body. Every manifest input and output must name a graph tensor, and `models.max_parameters` applies.
