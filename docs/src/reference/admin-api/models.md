...
It then probes the model on the node's default runtime for the manifest's format (`[models.default_runtime]`). The probe is five inferences over zero-filled inputs. Any named dimension runs at the manifest's `probe_dims`, 1 where it says nothing, and the binding is recorded in `stats.probe_dims`. The median must be within `models.max_probe_ms`, and the outputs must have the dtypes and shapes the manifest declares.
