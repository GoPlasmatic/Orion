...
The report has two sections and only the first gates. **Breaks** are numbered by their [checklist row](../../../releases/upgrade-to-1.0/index.md) and make the command exit non-zero, so `orion-server preflight || exit 1` is a deploy gate. Not every break has a row. `[channel-auth]` names a stored channel whose `auth` block no longer builds, which the next load would quarantine. A [`scheme`](../../channel-config/auth.md#the-scheme) that is not a scheme name is one cause. **Advisories** carry the same ids `lint` uses: `[engine.unguarded_validation]`, `[engine.group_continue_on_error]`, `[logic.tensor_operator_key]`. They name a stored workflow that serves correctly and says less than its author meant it to, and they never change the exit code. `[source.sql_key]` names a stored workflow holding a string `$sql` key, which is now reserved: the next update of that workflow is refused as uncompiled source. `[source.fragment_key]` does the same for a string `$use` or an object `$each`. For the tensor key, `preflight` reports more than `lint` does. On a stored estate a `{"shape": {"var": …}}` written before 1.8 cannot have meant a call. The dynamic objects are therefore listed here as well as the constant ones.
