...
| [Models](#models) | Register, admit, activate, version, import, and export ONNX models held in object storage |
...
admission worker then fetches the object through the connector, verifies the
digest, reads the graph (parameter and node counts, IR version, opset; every
manifest input and output must name a graph tensor, and
`models.max_parameters` applies), probes it on the node's default runtime
for the manifest's format (`[models.default_runtime]`; five inferences over
zero-filled inputs — the median must be within
`models.max_probe_ms`, and the outputs must have the dtypes and shapes the
manifest declares) and records the verdict on the row as `admission`
(`pending` → `passed` | `failed`, with the stage — `signature`, `gate`,
`head`, `size`, `fetch`, `digest`, `cache`, `parse`, `probe` — and reason)
and `stats`. Activation is refused until the verdict is `passed`. Every
route answers `400` on a node with `models.enabled = false`.
...
| GET | `/api/v1/admin/models/{id}` | Get the latest version, with this node's view under `health` (`pending`, `rejected`, `admitted`, `inactive`, `disabled`) |
...
| PATCH | `/api/v1/admin/models/{id}/status` | Activate (supersedes the previously active version; `409` until admission has passed) or archive (`409` while an active workflow names the model). `?dry_run=true` / `?reload=defer` — see [Status changes](#status-changes) |
