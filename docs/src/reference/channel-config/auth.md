...
**`jwt`** verifies a bearer token at ingress and exposes the **verified claims**, never the token, at `metadata.auth.claims.*`. `validation_logic`, `authorization_logic`, and every workflow task can read them there. That is the difference from fronting Orion with a gateway. A gateway can accept or reject, but it cannot give the workflow the identity (`sub`, roles) that per-user logic needs, except by forwarding spoofable headers.
