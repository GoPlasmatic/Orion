...
A model is an artifact and a *manifest*. The artifact is the ONNX graph. The manifest is a JSON document with `"abi": "orion:model@1.0.0"` that declares the model's name, its inputs and outputs, and the expressions that do the marshalling. Each input and output has a name, a dtype and a shape. A dimension is a count, or a **name** — `[1, 2, "H", 7]`. A name is for a graph exported with a variable axis: a batch, a sequence length, an image size. A name binds to what the call brings on its first occurrence. Every later occurrence must equal it, so `["N", 3]` on an output means the N the input had. Every axis the author did not name stays exact. One session serves every size; what bounds a call is `models.max_input_elements`, not the declaration. The admission probe needs concrete tensors. `probe_dims` says what to run each name at, 1 where it says nothing, and the value lands in `stats` beside `probe_ms`. One *adapter* per input is evaluated against the JSON the task hands over, producing that input's tensor. One *result* expression is evaluated against the output tensors, producing what the task writes.
...
Whatever the manifest declares, the node measures. Admission reads four things from the graph and records them as `stats`. They are the parameter and node counts, the operators it asks a runtime for, the IR version and the opset. The parameter count is every value the graph carries, wherever it carries it. That is its initializers, the tensors and lists of numbers its nodes hold in attributes, and the bodies of `If`, `Loop` and `Scan`. Re-exporting the same weights into another field of the format does not change it. The [`models.max_parameters`](../reference/configuration/models.md) ceiling applies to what was read, not what was claimed.
