...
A service in Orion is two JSON documents: a **workflow** that says what to do,
and a **channel** that says where to reach it. This tutorial creates both against
the server you just started, activates them, and calls the result.
...
Nothing here is compiled or deployed. Every step is one API call, and the
endpoint is live the moment you activate it.
...
The workflow parses the incoming payload, then writes one new field derived from
it. Two tasks, run in order:
...
It is saved as a **draft**. Drafts serve no traffic and never touch the running
engine, so nothing you do here can affect a live endpoint.
...
Activation triggers a hot reload: Orion builds a new engine and swaps it in.
In-flight requests finish on the engine they started with, so there is no restart
and no dropped traffic.

## 3. Create and activate the channel

The channel is the endpoint. It names the route, the methods it answers, and the
workflow it runs:
...
A channel can only be activated once its workflow is active — the endpoint can
never point at logic that is not serving.
...
- [Your First Connector](./first-connector.md) — the same shape of service, but
  reading and writing a real PostgreSQL database.
- [Test & Promote a Service](./test-and-promote.md) — test this workflow offline,
