...
| **Expression budget** | `engine.ops_budget` whenever expressions come from someone other than the operator — tenant rules, competitor-written model adapters. Size it from the heaviest legitimate expression: a condition that crosses it fails closed to `false`, silently. | [Configuration › Engine](../reference/configuration.md#engine) |
