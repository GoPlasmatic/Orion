...
- [Rollout](../../reference/workflows.md#rollout) percentages across the active versions of one workflow that do not sum to 100. Under, and part of the traffic matches no version; over, and the later versions are unreachable. Either way the whole channel is quarantined rather than serving a rollout that silently misroutes.
...
If the channel is `active` but the request still `404`s, the route pattern is the suspect. Routes match byte-exactly, including case: `/Orders` does not match a channel declaring `/orders`.
...
**What to do.** Compare against the operator catalogue in [Expression language](../../reference/expressions.md), and dry-run the workflow before activating it.
...
|---|---|---|
| An unknown config key | The key does not exist, or was renamed | The error names the nearest real key. See [Server configuration](../../reference/configuration/index.md) |
...
- [Monitor and alert](../run/monitoring.md): the signals that surface these before a caller does.
- [Traces and async processing](../run/traces.md): the queue and DLQ in full.
- [Timeouts, retries and circuit breakers](../run/failure-handling.md): the controls behind several entries above.
- [Errors and response envelopes](../../reference/errors.md): every error code and what it means.
