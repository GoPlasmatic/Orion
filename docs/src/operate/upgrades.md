...
- [Upgrading to 1.8.0](./upgrading-to-1.8.md): from 1.7.x. A minor release
  that adds ONNX models (off by default) and turns on the JSONLogic tensor
  operators and the evaluation budget: one expand-only migration, one thing to
  review — a stored `map` mapping that emits a literal object keyed `shape`,
  `full`, `cast`, `pad`, `crop`, `concat` or `stack` now needs the `$` escape,
  and `preflight` lists every such key — and one setting,
