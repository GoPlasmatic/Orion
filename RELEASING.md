...
`v1.0.0-rc.1`) starts two independent pipelines, both gated on a successful
...
5. **Record:** commit the run outputs under
   `tests/benchmark/results/v1.0.0/` (the per-scenario `.txt` files plus
   `SUMMARY.md`, following the existing `results/v0.2.0/` layout).
