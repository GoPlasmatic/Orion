...
--
-- The width change is the second half: every other backend stores the scope
-- as unbounded text, and a 32-char ceiling only on MySQL is a truncation
-- waiting for a longer scope name.

ALTER TABLE `config_epoch` ADD COLUMN `epoch_scope_at` bigint NOT NULL DEFAULT 0;
ALTER TABLE `config_epoch` MODIFY COLUMN `epoch_scope` VARCHAR(64) NOT NULL DEFAULT '';
