...
pub(crate) async fn create_connector(
    ...
    // §2.6: the connector row and its audit row commit together. A connector
    // has no draft state — the row is live the moment it commits, and it
    // carries credentials — so this is the audited path, not the queue.
    //
    // The id is generated by the INSERT when the request does not carry one,
    // so the event is re-addressed once the row exists — see
    // `AuditedWrite::addressed_to`.
    let mut write = super::audited_write(&state, &principal, "create", "connector", "").await?;
    let connector = state.repos.connectors.create_tx(write.tx(), &req).await?;
    write.addressed_to(&connector.id);
    write.commit().await?;
...
pub(crate) async fn update_connector(
    ...
    // §2.6: the connector row and its audit row commit together — see
    // `create_connector` for why a connector write is always the audited path.
    let mut write = super::audited_write(&state, &principal, "update", "connector", &id).await?;
    let connector = state
        .repos
        .connectors
        .update_tx(write.tx(), &id, &req)
        .await?;
    write.commit().await?;
    // Eviction runs after the commit, like the reload: it drops live TCP
    // connections, which a rollback could not put back.
    //
...
pub(crate) async fn delete_connector(
    ...
    let connector = state.repos.connectors.get_by_id(&id).await?;
    // §2.6: the delete and its audit row commit together — see
    // `create_connector` for why a connector write is always the audited path.
    let mut write = super::audited_write(&state, &principal, "delete", "connector", &id).await?;
    state.repos.connectors.delete_tx(write.tx(), &id).await?;
    write.commit().await?;
    // After the commit: eviction drops live TCP connections, which a rollback
    // could not put back.
