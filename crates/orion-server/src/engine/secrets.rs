...
impl ResolvedSecrets {
    ...
    pub async fn resolve(declared: &SecretsConfig) -> Result<Self, OrionError> {
        // The process-wide registry, borrowed rather than built. The loop
        // stays per-entry because the error has to name the entry
        // (`secrets.partner_hmac`), not the reference it holds.
