...
pub struct TraceQueueConfig {
    ...
    /// How often the trace cleanup task runs, in seconds. Audit-log cleanup
    /// has its own cadence in `[audit]` — pre-1.0 this one key drove both.
...
impl Default for TraceQueueConfig {
    fn default() -> Self {
        Self {
            workers: 4,
            buffer_size: 1000,
...
impl TraceQueueConfig {
    pub(crate) fn validate(&self) -> Result<(), OrionError> {
        require_nonzero(self.workers as u64, "trace_queue.workers")?;
        require_nonzero(self.buffer_size as u64, "trace_queue.buffer_size")?;
        require_nonzero(
        ...
        require_nonzero(
            ...
            "trace_queue.shutdown_timeout_secs",
        )?;
        ...
        if self.dlq_max_retries < 1 {
            return Err(OrionError::Config {
                message: "trace_queue.dlq_max_retries must be >= 1 \
                          (set trace_queue.dlq_retry_enabled = false to disable retries)"
                    .to_string(),
            });
        }
        ...
        if self.dlq_max_retries > 16 {
            return Err(OrionError::Config {
                message: "trace_queue.dlq_max_retries must be <= 16 (backoff is \
        ...
        if self.dlq_batch_size < 1 {
            return Err(OrionError::Config {
                message: "trace_queue.dlq_batch_size must be >= 1".to_string(),
            });
        }
        require_nonzero(self.dlq_lease_secs, "trace_queue.dlq_lease_secs")?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_validate_default_passes() {
        assert!(TraceQueueConfig::default().validate().is_ok());
    }

    #[test]
    fn test_validate_rejects_zero_dlq_max_retries() {
        let config = TraceQueueConfig {
            dlq_max_retries: 0,
            ..Default::default()
        };
