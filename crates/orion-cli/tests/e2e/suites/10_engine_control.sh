...
test_engine_reload_updates_count() {
    ...
    cli engine status
    assert_json_eq "$CLI_OUTPUT" '.data.active_workflows' '0'
    ...
    cli engine status
    assert_json_eq "$CLI_OUTPUT" '.data.active_workflows' '2'
}
...
test_engine_channels_reported() {
    ...
    cli engine status
    local channels
    channels=$(echo "$CLI_OUTPUT" | jq -r '.data.channels | sort | join(",")')
